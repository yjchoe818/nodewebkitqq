...
var HTML5QQ = {
	...
	httpRequest: function(method, action, query, urlencoded, callback, timeout){
		...
		var qunpattern=/http:\/\/*.qun.qq.com\/*/;
		if("POST" == method && urlencoded){
			options.headers['Content-Type'] = 'application/x-www-form-urlencoded; charset=UTF-8';
            options.headers['Content-Length']= Buffer.byteLength(query);
		}
        if(url.indexOf("http://s.web2.qq.com/api/")===0){
        	options.headers.Origin='http://d.web2.qq.com';
        	options.headers.Referer='http://d.web2.qq.com/proxy.html?v=20110412001&callback=1&id=1';
        }
        else if(url.indexOf("https://d.web2.qq.com/channel/")===0||url.indexOf("http://d.web2.qq.com/channel/")===0){
        	options.headers.Origin='http://d.web2.qq.com';
        	options.headers.Referer='http://d.web2.qq.com/proxy.html?v=20130916001&callback=1&id=2';
        }
        else if(url.indexOf("http://web.qq.com/cgi-bin/")===0){
        	options.headers.Origin='http://d.web2.qq.com';
        	options.headers.Referer='http://d.web2.qq.com/proxy.html?v=20110412001&callback=1&id=3';
        }
        else if(url.indexOf("http://weboffline.ftn.qq.com/")===0||url.indexOf("http://up.web2.qq.com/")===0){
        	options.headers.Origin='http://web.qq.com';
        	options.headers.Referer='http://web.qq.com/';
        }
        else if(qunpattern.test(url)){
        	options.headers.Referer='http://web.qq.com/';
        }
        options.timeout=timeout;
        if(url.indexOf("http://ptlogin4.web2.qq.com/check_sig")===0){//使用request返回404Forbidden，不知为什么
            httpRequest.get(httpOptions,function(response){
              	if(response.headers&&response.headers['set-cookie']){
              		var cookies=response.headers['set-cookie'];
              		if (Array.isArray(cookies)) {
              			HTML5QQ.all_cookies=HTML5QQ.all_cookies.concat(cookies);
              		}
                    else {
                    	HTML5QQ.all_cookies.push(cookies);
                   }              	
                }
                var body='';
           ...
           if(error) {
              ...
              if(callback){
              	if(response.headers&&response.headers['set-cookie']){
              		var cookies=response.headers['set-cookie'];
              		if (Array.isArray(cookies)) {
              			cookies.forEach(HTML5QQ.processCookie);
              			HTML5QQ.all_cookies=HTML5QQ.all_cookies.concat(cookies);
              		}
                    else {
                    	HTML5QQ.processCookie(cookies);
                    	HTML5QQ.all_cookies.push(cookies);
                    }
              	}
		    	callback(body);
	...
	processCookie:function(cookieStr){
	   var cookies=cookie.parse(cookieStr);
      if(cookies.skey) HTML5QQ.skeyCookie=cookies.skey;
      if(cookies.ptwebqq) HTML5QQ.ptwebqqCookie=cookies.ptwebqq;
      if(cookies.uin) HTML5QQ.uinCookie=cookies.uin;
	},
	...
	getVerifyCode: function(qq){
		...
		HTML5QQ.createJs("https://ssl.ptlogin2.qq.com/check?uin="+qq+"&appid=501004106&r="+Math.random(),function(code){
			...
			var uin=query[2].split("',")[0];
			HTML5QQ.checkVerifyCode(query[0], query[1],uin);
		});
	},
	...
	getVerifyCodeImg: function(verifyCode){
		chrome.extension.sendMessage("getVCI"+HTML5QQ.qq+'&'+verifyCode);
	},
		...
		HTML5QQ.createJs("https://ssl.ptlogin2.qq.com/login?u="+HTML5QQ.qq+"&p="+HTML5QQ.encodedPassword+"&verifycode="+HTML5QQ.verifyCode.toUpperCase()+"&webqq_type=10&remember_uin=1&login2qq=1&aid=501004106&u1=http%3A%2F%2Fw.qq.com%2Fproxy.html%3Flogin2qq%3D1%26webqq_type%3D10&h=1&ptredirect=0&ptlang=2052&daid=164&from_ui=1&pttype=1&dumy=&fp=loginerroralert&action=0-42-19466&mibao_css=m_webqq&t=1&g=1&js_type=0&js_ver=10063&login_sig="+HTML5QQ.sig, function(code){
			...
			if(code.indexOf("登录成功") != -1){
				...
				HTML5QQ.httpRequest('GET', jmp, null, false, function(result){
					HTML5QQ.skey = HTML5QQ.skeyCookie;
						if(HTML5QQ.debug){
			 				HTML5QQ.outputDebug("login: skey("+HTML5QQ.skey+")");
						}
					HTML5QQ.getVfwebqq(HTML5QQ.ptwebqqCookie, status);
						if(HTML5QQ.debug){
			 				HTML5QQ.outputDebug("login: ptwebqq("+HTML5QQ.ptwebqq+")");
						}
					HTML5QQ.qq = parseInt(HTML5QQ.uinCookie.substr(1));
						if(HTML5QQ.debug){
			 				HTML5QQ.outputDebug("login: uin("+HTML5QQ.uinCookie+")");
						}
				});
			}
			else{
	...
	getFriendsInfo: function(){
		...
		var r = '{"h":"hello","vfwebqq":"'+HTML5QQ.vfwebqq+'","hash":"'+HTML5QQ.hash(HTML5QQ.qq+'',HTML5QQ.ptwebqq)+'"}';
	...
	getPersonal: function(){
		...
		HTML5QQ.httpRequest("GET", url, null, false, function(result){
			try{
				...
				if(HTML5QQ.debug){
			 		HTML5QQ.outputDebug("getPersonal: Failed, try again."+e.message);
				}
